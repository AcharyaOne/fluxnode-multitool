...
function install_daemon() {
   ...
   if [[ "$(lsb_release -cs)" == "xenial" ]]; then
	...
	echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/flux-archive-keyring.gpg] https://apt.fluxos.network/ noble main" | sudo tee /etc/apt/sources.list.d/flux.list  > /dev/null 2>&1
        echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/flux-archive-keyring.gpg] https://apt.runonflux.io/ noble main" | sudo tee --append /etc/apt/sources.list.d/flux.list  > /dev/null 2>&1
